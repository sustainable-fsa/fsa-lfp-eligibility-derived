...
- [`fsa-lfp-eligibility-derived.json`](https://data.sustainable-fsa.com/fsa-lfp-eligibility-derived/fsa-lfp-eligibility-derived.json)
...
The derived records are mirrored in **both** places: the CSV, Parquet,
and JSON are committed to this repository as well as published to S3, so
...
9.  **Exports** `fsa-lfp-eligibility-derived.csv`,
    `fsa-lfp-eligibility-derived.parquet`, and the browser-optimized
    `fsa-lfp-eligibility-derived.json`, and publishes to S3.
...
[`fsa-lfp-eligibility-derived.json`](https://data.sustainable-fsa.com/fsa-lfp-eligibility-derived/fsa-lfp-eligibility-derived.json)
holds the same records as the CSV and Parquet — derived from the
identical validated table in the same run — restructured for direct use
in a browser. It is what the eligibility dashboard loads, not an
...
- **Column-oriented**: one array per field with one entry per record,
  ...
  so the file gzips from ~11.7 MB to roughly 350 KB over the wire.
- **Dictionary-coded strings**: the 15 pasture types, 2,896 FSA county
  codes, 2,968 Census FIPS codes, 7 qualifying drought events, and 4
  USDM county aggregations each appear once in a lookup array; the
  record arrays hold integer indices into them.
- **Compact dates**: each `Qualifying Date` is a day-of-year (`qy`) plus
  ...
  carries qualifying dates there too — 12% of the events here — so the
...
tabular files carry, for the same reason (see *Two county keys* below).
It also carries the `source` dimension, so every record names which of
the four USDM county-aggregation conventions produced it and all four
reanalyses are readable from the one file. There are no nulls anywhere:
every field is populated for every record, which the run asserts before
writing.

There are no payment columns. `Drought Factor` is [not the payable
amount](#fsa-lfp-eligibility-derivedcsv-and-parquet), and neither the
*Maximum Eligible Payment Months* cap nor the capped figure is in this
archive in any format.
